...
if ($page_no < 1) {
    ...
    $next_page = $page_no + 1;
    $adjacents = "2";
    $stmt = mysqli_prepare($con, "SELECT COUNT(*) As total_records FROM tblfood WHERE CategoryName = ?");
  ...
  $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1; // total page minus 1
      $stmt = mysqli_prepare($con, "SELECT * FROM tblfood WHERE CategoryName = ? LIMIT ?, ?");
      mysqli_stmt_bind_param($stmt, "sii", $cid, $offset, $total_records_per_page);
      mysqli_stmt_execute($stmt);
